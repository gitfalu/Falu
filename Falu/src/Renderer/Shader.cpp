...
namespace Falu
{
	...
	bool Shader::LoadVartexShader(ID3D11Device* device, const std::wstring& filename, const std::vector<ShaderDefine>& define)
	{
	...
	Shader* ShaderManager::LoadShader(ID3D11Device* device, const std::string& name, const std::wstring& vsFile, const std::wstring& psFile, const D3D11_INPUT_ELEMENT_DESC* layout, UINT numElements)
	{
		...
		// VertexShader
		if (!shader->LoadVartexShader(device, vsFile))
			return nullptr;
		// PixelShader
